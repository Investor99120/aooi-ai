...
Source prompt（来源 prompt）：frl_prompt_003

First batch routing（第一批路由）：

- FAQ cluster（FAQ 内容簇）：future_faq_adapter, future_manual_review_gate
- Claim-safe terminology page（claim-safe 术语页）：future_claim_safety_adapter, future_manual_review_gate
- Product routine support block（产品日常使用支持模块）：future_shopify_adapter, future_manual_review_gate

Deferred routing（暂缓路由）：

- Citation-ready source page（可引用来源页）：future_citation_source_adapter, future_manual_review_gate
- Social video brief（社媒视频简报）：future_social_video_adapter, future_video_factory_adapter, future_manual_review_gate
- Educational guide（教育指南）：future_educational_guide_adapter, future_manual_review_gate

Future adapter targets（未来适配器目标）：

- FAQ cluster（FAQ 内容簇）：future_faq_adapter, future_manual_review_gate
- Claim-safe terminology page（claim-safe 术语页）：future_claim_safety_adapter, future_manual_review_gate
- Shopify product-page routine section（Shopify 产品页日常使用区块）：future_shopify_adapter, future_manual_review_gate
- Email education flow（邮件教育流程）：future_email_adapter, future_manual_review_gate
- Product routine support block（产品日常使用支持模块）：future_shopify_adapter, future_manual_review_gate
- Educational guide（教育指南）：future_educational_guide_adapter, future_manual_review_gate
- Social video brief（社媒视频简报）：future_social_video_adapter, future_video_factory_adapter, future_manual_review_gate
- Citation-ready source page（可引用来源页）：future_citation_source_adapter, future_manual_review_gate
